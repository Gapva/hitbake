...
| pros | cons |
| - | - |
| extremely performant since it is pre-rendered | slightly longer load times (can be cached, however, to speed things up even more) |
| sounds are guaranteed to remain in perfect sync even while other performance suffers | can use lots of disk space if cached (extremely variable) |
